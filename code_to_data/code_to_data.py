...
Compare Dec comments to babel audit data:
...
Compare Dec comments to SQL code:
...
Since Dec comments from A.F., no codes have been added::

    >>> g_f_ank - set(LabReview.fasting_glucose_loinc)
    set()
    >>> g_r_ank - set(LabReview.random_glucose_loinc)
    set()

Various codes have been pruned::

    >>> print(', '.join(sorted(set(LabReview.fasting_glucose_loinc) - g_f_ank)))
    ... # doctest: +NORMALIZE_WHITESPACE
    10450-5, 14769-4, 14770-2, 14771-0, 1493-6,
    1550-3, 1554-5, 1555-2, 1556-0, 1557-8, 1558-6,
    17865-7, 41604-0, 76629-5, 77145-1

    >>> print(', '.join(sorted(set(LabReview.random_glucose_loinc) - g_r_ank)))
    ... # doctest: +NORMALIZE_WHITESPACE
    10449-7, 12610-2, 12646-6, 14749-6,
    1504-0, 1506-5, 1517-2, 1521-4, 1530-5,
    20436-2, 2339-0, 2339-0:N, 2339-0:T, 2342-4, 2345-7, 26779-9, 27353-2,
    43151-0, 50586-7, 50587-5, 50588-3, 50589-1, 50608-9, 50673-3, 53553-4, 55399-0,
    6777-7, 72171-2, 81637-1, LP31895-3, LP42107-0, LP71758-4
...
class LabNotes(object):
    ...
    def categorize(cls, term):
        ...
        if (term.sheet == 'Glucose' and note in cls.ok_glucose):

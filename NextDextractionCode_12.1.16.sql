...
select l.PATID, l.LAB_ORDER_DATE, row_number() over (partition by l.PATID order by l.LAB_ORDER_DATE asc) rn into #A1c_initial 
...
and (datediff(yy,d.BIRTH_DATE,l.LAB_ORDER_DATE) <= 89 and datediff(yy, d.BIRTH_DATE,l.LAB_ORDER_DATE) >=18);
...
---------------------------------------------------------------------------------------------------------------
-- Get all labs for each patient sorted by date:
select l.CAP_ID as PATID, l.LAB_ORDER_DATE, row_number() over (partition by l.CAP_ID order by l.LAB_ORDER_DATE asc) rn into #FG_initial 
...
---------------------------------------------------------------------------------------------------------------
-- Get all labs for each patient sorted by date:
select l.CAP_ID as PATID, l.LAB_ORDER_DATE, row_number() over (partition by l.CAP_ID order by l.LAB_ORDER_DATE asc) rn into #RG_initial 
from capricorn.dbo.CAP_LABS l 
join capricorn.dbo.ENCOUNTER e
on l.ENCOUNTERID=e.ENCOUNTERID 
join capricorn.dbo.DEMOGRAPHIC d
...
select e.PATID, l.ADMIT_DATE, row_number() over (partition by l.PATID order by l.ADMIT_DATE asc) rn into #Visits_initial 
...
where
	...
	and datediff(yy, l.ADMIT_DATE,d.BIRTH_DATE) <=89 and datediff(yy, l.ADMIT_DATE,d.BIRTH_DATE) >=18;
...
and e.ENC_TYPE in ('IP', 'EI', 'AV', 'ED')
and datediff(yy, d.BIRTH_DATE, a.RX_ORDER_DATE) <=89 and datediff(yy, d.BIRTH_DATE, a.RX_ORDER_DATE) >=18;
...
and e.ENC_TYPE in ('IP', 'EI', 'AV', 'ED')
and datediff(yy, a.RX_ORDER_DATE,d.BIRTH_DATE) <=89 and datediff(yy, a.RX_ORDER_DATE,d.BIRTH_DATE) >=18;
...
---------------------------------------------------------------------------------------------------------------
select y.PATID, y.MedDate as EventDate into #InclusionMeds_final
from 
...
select y.PATID, y.MedDate as EventDate into #InclusionUnderRestrictionMeds_final
from
(select x.PATID, x.MedDate,row_number() over(partition by x.PATID order by x.MedDate asc)rn
from
(select a.PATID, a.MedDate  
...
---------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------
-----                          Part 2: Defining Denominator or Study Sample                               -----  
--------------------------------------------------------------------------------------------------------------- 
---------------------------------------------------------------------------------------------------------------
...
join capricorn.dbo.DEMOGRAPHIC d
on e.PATID=d.PATID
where e.ENC_TYPE in ('IP', 'EI', 'AV', 'ED') and (datediff(yy,d.BIRTH_DATE,e.ADMIT_DATE) <= 89 and datediff(yy, d.BIRTH_DATE,e.ADMIT_DATE) >=18);
...
select dia.PATID, dia.ADMIT_DATE into #Miscarr_Abort
	...
	on e.PATID=d.PATID
		...
		and (datediff(yy,d.BIRTH_DATE,dia.ADMIT_DATE) <= 89 and datediff(yy, d.BIRTH_DATE,dia.ADMIT_DATE) >=18);
...
select p.PATID, p.ADMIT_DATE into #DelivProc
	...
	on e.PATID=d.PATID	
		...
		and (datediff(yy,d.BIRTH_DATE,p.ADMIT_DATE) <= 89 and datediff(yy, d.BIRTH_DATE,p.ADMIT_DATE) >=18);
